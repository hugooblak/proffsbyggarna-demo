...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'proffsbyggarna-leads' ),
					'singular_name' => __( 'Lead', 'proffsbyggarna-leads' ),
					'edit_item'     => __( 'Förfrågan', 'proffsbyggarna-leads' ),
					'search_items'  => __( 'Search leads', 'proffsbyggarna-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'proffsbyggarna-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'proffsbyggarna-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'proffsbyggarna-leads' ),
			'npl_phone'    => __( 'Phone', 'proffsbyggarna-leads' ),
			'npl_service'  => __( 'Needs', 'proffsbyggarna-leads' ),
			'npl_timeline' => __( 'When', 'proffsbyggarna-leads' ),
			'npl_zip'      => __( 'ZIP', 'proffsbyggarna-leads' ),
			'npl_source'   => __( 'Source', 'proffsbyggarna-leads' ),
			'npl_status'   => __( 'Status', 'proffsbyggarna-leads' ),
			'date'         => __( 'Received', 'proffsbyggarna-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'proffsbyggarna-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'proffsbyggarna-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'proffsbyggarna-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'proffsbyggarna-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'proffsbyggarna-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'proffsbyggarna-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'proffsbyggarna-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'proffsbyggarna-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'proffsbyggarna-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'proffsbyggarna-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'proffsbyggarna-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'proffsbyggarna-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
