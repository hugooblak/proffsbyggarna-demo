...
add_filter(
	...
	function ( $exporters ) {
		$exporters['proffsbyggarna-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'proffsbyggarna-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'proffsbyggarna-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'proffsbyggarna-leads',
						'group_label' => __( 'Quote requests', 'proffsbyggarna-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['proffsbyggarna-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'proffsbyggarna-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
