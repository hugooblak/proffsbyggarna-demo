 ...
 * or photo. They call pr_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Proffsbyggarna AB.
 *
 ...
 * @return string
 */
function pr_sample_note() {
	return pr_lead( 'ar_riktig' ) ? '' : ' <span class="pr-sample">(platshållare)</span>';
}
 ...
 * @return string File name for pr_lead_img().
 */
function pr_lead_bild( $i ) {
	$bilder = (array) pr_lead( 'bilder', array() );
 ...
 * @return string
 */
function pr_ring_label() {
	$namn = pr_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, pr_lead( 'telefon' ) ) : 'Ring ' . pr_lead( 'telefon' );
}
