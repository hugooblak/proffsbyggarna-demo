<?php
/**
 * Proffsbyggarna AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Proffsbyggarna AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through pr_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'proffsbyggarna', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'proffsbyggarna' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'proffsbyggarna' ) ) );

		register_block_pattern_category( 'proffsbyggarna-sections', array( 'label' => __( 'Proffsbyggarna AB sections', 'proffsbyggarna' ) ) );
		register_block_pattern_category( 'proffsbyggarna-pages', array( 'label' => __( 'Proffsbyggarna AB page layouts', 'proffsbyggarna' ) ) );
	}
);
